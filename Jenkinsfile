                ...
                stage("sandbox - validate deployment"){
                    ...
                    steps {
                        sh '''
                           ...
                           DELAY=5
                           MAX_COUNTER=60
                           echo "Validating deployment of ${APP_NAME}-nginx in project ${TARGET_NAMESPACE}"
                           LATEST_DC_VERSION=\$(oc get dc ${APP_NAME}-nginx -n ${TARGET_NAMESPACE} --template='{{ .status.latestVersion }}')
                           RC_NAME=${APP_NAME}-nginx-\${LATEST_DC_VERSION}
                           set +e
